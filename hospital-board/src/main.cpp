...
//--- MQTT Configuration ---
const char* mqtt_server = "930cae777a53421eaa614ec138961b73.s1.eu.hivemq.cloud";
const int mqtt_port = 8883;
const char* mqtt_user = "health-gard";
const char* mqtt_pass = "Health-gard##12";
const char* mqtt_topic = "room/env";
const char* mqtt_topic_sub = "room/control";
...
// Status Indicators
const int GREEN_LED_PIN = 32;  // LED (normal status)
const int RED_LED_PIN = 33;    // LED (alert status)
const int BUZZER_PIN = 26;     // Buzzer/Audio alarm

// Input
const int BUTTON_PIN = 35;     // Emergency SOS button
const int BUTTON_SHUTDOWN_PIN = 34;  // button arrêt alarme
...
void checkSafetyLimits() {
  ...
  // Seulement détecter si température élevée
  if (currentSensorData.temperature > TEMP_ALERT_THRESHOLD) {
    systemState.isAlertActive = true;

    if (!wasAlertActive) {
      Serial.println("[ALERT] Temperature threshold exceeded! Waiting for MQTT confirmation...");
    }
  }
  else {
...
void checkEmergencyButton() {
  ...
  if (buttonState == LOW && !systemState.buttonWasPressedLastCycle) {
    ...
    remoteAlertActive = false;        // 🔥 STOP MQTT ALARM
    manualAlarmOverride = true;       // 🔥 priorité bouton

    digitalWrite(BUZZER_PIN, LOW);
    systemState.buzzerActive = false;

    Serial.println("\n!!! [EMERGENCY] BUTTON → ALARM STOPPED !!!\n");
...
void updateAlertSystem() {
  // Update LED status
  if ((remoteAlertActive || systemState.isEmergencyActive) && !manualAlarmOverride) {
...
void reconnectMQTT() {
  while (!client.connected() && WiFi.isConnected()) {
    ...
    if (client.connect(clientId.c_str(), mqtt_user, mqtt_pass)) {
      ...
      Serial.println("[MQTT] Subscribed to hospital/control");
      // Subscribe to potential control topics (if needed in future)
      //client.subscribe("hospital/sensor/control");
    }
    else {
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (strncmp((char*)payload, "ALERT", 5) == 0) {
    Serial.println("[ACTION] ALERT reçu → Activation alarme");

    remoteAlertActive = true;
    manualAlarmOverride = false;

    // Activer immédiatement buzzer + LED
    soundBuzzer(1000);
  }

  else if (message == "STOP") {
    Serial.println("[ACTION] STOP reçu → Désactivation alarme");
